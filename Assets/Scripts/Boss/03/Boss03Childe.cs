...
public class Boss03Childe : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collision.tag == "Bullet")
        {
            ...
            if (!FindObjectOfType<Boss03APattern>().constanFlg) FindObjectOfType<Boss03HitJugement>().hp -= damage;
            //hp-=武器の攻撃力
            if (!Boss03Manager.GetDeadPerformance()) DamageText.GetInstance.DrawDamage(transform.position, damage.ToString("F0"));

            if (FindObjectOfType<Boss03HitJugement>().hp >= 1.0f || FindObjectOfType<Boss03HitJugement>().hp > 0.1f)
            {
    ...
    void Update()
    {
        ...
        if (TimeManager.GetInstance.timeScale >= 1.0f)
        {
            ...
            for (int i = 0; i < NumFollowers; i++)
            {
                ...
                //4方向ショット1回の時//
                if (Boss03Manager.GetMode()==6)
                {
                    FollowerList[i].transform.position = Boss.transform.position;
                }
                //4方向ショット1回の時//
                if (Boss03Manager.GetMode() == 7)
                {
                    FollowerList[i].transform.position = Boss.transform.position;
                }
                //円形ショットの時//
