...
public class EnemyDivisionEightDir : MonoBehaviour
{
    [Header("8方向分裂の敵ですわ(ﾟДﾟ )")]
    ...
    void Division()
    {
        ...
        if (deadF)
        {
            pos1.y = pos1.y + 1.5f;
            pos2.y = pos2.y - 1.5f;
            Instantiate(childe[0], pos1, Quaternion.identity);
            Instantiate(childe[1], pos2, Quaternion.identity);
            Destroy(gameObject);
